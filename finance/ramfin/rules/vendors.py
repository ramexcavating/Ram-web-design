    ...
    with open(p, newline="", encoding="utf-8-sig") as fh:
        for r in csv.DictReader(fh):
            ...
            if not name:
                continue
            conn.execute(
                "INSERT INTO vendors(name, norm_name, aliases, email_domain, default_cost_code, default_terms_days, category, critical) "
                "VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(norm_name) DO UPDATE SET aliases=excluded.aliases, email_domain=excluded.email_domain, "
                "default_cost_code=excluded.default_cost_code, default_terms_days=excluded.default_terms_days, category=excluded.category, critical=excluded.critical",
