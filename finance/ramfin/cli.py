"""ramfin command line.
  ...
  ramfin ingest [--folder PATH]        pull new documents from mailboxes, IMAP, CamScanner folder (or a local folder)
...
def cmd_weekly(a, settings):
    ...
    if a.send:
        ...
        graph = _graph()
        att = [(Path(out["xlsx"]).name, Path(out["xlsx"]).read_bytes()), (Path(out["markdown"]).name, Path(out["markdown"]).read_bytes())]
        ...
        if not a.local:
            ...
            filer.file(out["markdown"], FilingDecision(f"{sp['cashflow']}/01_WEEKLY_MANAGERS_REPORT", Path(out["markdown"]).name))
...
def main(argv=None):
    ...
    s = sub.add_parser("weekly"); s.add_argument("--send", action="store_true"); s.add_argument("--local", action="store_true"); s.add_argument("--no-deferrals", action="store_true"); s.set_defaults(fn=cmd_weekly)
